...
\begin{remark}[Angle between two states]
    ...
    \end{equation*}
    The angle is non-negative, symmetric in its inputs, and is equal to zero if and only if $\psi = \phi$. This follows triangle inequality.
\end{remark}

\begin{figure}[H]
    \centering
    % \includegraphics[width=\textwidth]{fidelity_plot.png}
    \resizebox{\textwidth}{!}{\input{images/fidelity_plot.tikz}}
    \caption{Fidelity vs Number of generations}
    \label{fig:fidelity}
\end{figure}
